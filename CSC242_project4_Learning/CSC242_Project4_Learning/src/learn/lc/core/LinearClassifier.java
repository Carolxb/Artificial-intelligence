...
abstract public class LinearClassifier {
	 ...
	 * See AIMA p. 724.
	 */
	public void train(List<Example> examples, int nsteps, LearningRateSchedule schedule, double[] accuracies) {
		...
		for (int i=1; i <= nsteps; i++) {
			...
			this.trainingReport(examples, i, accuracies);
		}
	}
	 ...
	 */
	public void train(List<Example> examples, int nsteps, double constant_alpha, double[] accuracies) {
	 ...
	 */
	protected void trainingReport(List<Example> examples, int stepnum, double[] accuracies) {
		double accuracy = accuracy(examples);
		accuracies[stepnum] = accuracy;
		if(stepnum % 100 == 0) System.out.println(stepnum + "\t" + accuracy);
