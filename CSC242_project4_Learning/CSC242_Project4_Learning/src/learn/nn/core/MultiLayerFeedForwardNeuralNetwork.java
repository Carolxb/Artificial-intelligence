...
abstract public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	...
	public void backprop(Example example, double alpha) {
		...
		for(int j=0; j<example.outputs.length; j++) {
			
			LogisticUnit unit = (LogisticUnit) this.layers[getNumLayers() - 1][j];
			double in_j = unit.getInputSum(); 
			double y_j = example.outputs[j];
			double a_j = unit.activation(in_j);
			//Delta[j] <- g'(in_j) \times (y_j - a_j)
			this.getOutputUnits()[j].delta = unit.activationPrime(in_j) * (y_j - a_j);
		
		}
		
		// for l = L-1 to 1 do
		for (int l=this.getNumLayers()-2; l>0; l--) {
			...
			for (int i=0; i<this.layers[l].length; i++) {
				...
				double in_i = unit.getInputSum();
				double sum = 0;
				for (Connection connection : unit.outgoingConnections) {
					sum += connection.weight*((LogisticUnit)connection.dst).delta;
				}
				this.getLayerUnits(l)[i].delta = unit.activationPrime(in_i) * sum;
			}
		}
		
		// for each weight w_ij in network do
		for (int i=1; i<this.getNumLayers(); i++) {
			for (int j=0; j<this.getLayerUnits(i).length; j++) {
				//w_ij <- w_ij + alpha * a_i * delta_j
				for (Connection connection : ((LogisticUnit) this.layers[i][j]).incomingConnections) {
					double a_i = connection.src.getOutput();
					double delta_j = ((LogisticUnit) this.layers[i][j]).delta;
					connection.weight += alpha * a_i * delta_j;
				}
			}
		}
